...
export const fetchRentals = createAsyncThunk('rentals/fetchRentals', async () => {
  ...
  try {
    const response = await axios.get(`https://rails-appointwheels-bold-butterfly-8850.fly.dev/api/v1/users/${user.id}/rentals`, {
...
export const createRental = createAsyncThunk('rentals/createRental', async (rental) => {
  ...
  const authTokenData = sessionStorage.getItem('authToken');
  try {
    const response = await axios.post(`https://rails-appointwheels-bold-butterfly-8850.fly.dev/api/v1/users/${user.id}/rentals`, rental, {
      headers: { Authorization: authTokenData },
    });
...
export const cancelRental = createAsyncThunk('rentals/deleteRental', async (id) => {
  ...
  const authTokenData = sessionStorage.getItem('authToken');
  try {
    await axios.delete(`https://rails-appointwheels-bold-butterfly-8850.fly.dev/api/v1/users/${user.id}/rentals/${id}`, {
      headers: { Authorization: authTokenData },
      withCredentials: true,
    });
